...
def validate_against_schema(data_path: Path, schema_path: Path, label: str) -> dict[str, Any]:
...
def validate_normalized_number(value: Any, label: str) -> None:
    if not isinstance(value, (int, float)):
        fail(f"{label} must be a number, got {type(value).__name__}")
